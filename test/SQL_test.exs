defmodule EctoRawSQLHelpers.SQLTest do
  ...
  test "query should return list of maps" do
    query_result = SQL.query(EctoRawSQLHelpers.PostgresRepoForTest, "SELECT * FROM generate_series(1, 2)")
    assert query_result == [%{"generate_series" => 1}, %{"generate_series" => 2}]

    query_result = SQL.query(EctoRawSQLHelpers.PostgresRepoForTest, "SELECT * FROM generate_series(1, 2)", [], [column_names_as_atoms: true])
    assert query_result == [%{generate_series: 1}, %{generate_series: 2}]
  end

  test "query_get_single_return should return map" do
    query_result = SQL.query_get_single_result(EctoRawSQLHelpers.PostgresRepoForTest, "SELECT *, 'a' as a FROM generate_series(1, 1)")
    assert query_result == %{"generate_series" => 1, "a" => "a"}
  end

  test "query_get_single_return should nil when response is empty" do
    SQL.affecting_statement(EctoRawSQLHelpers.PostgresRepoForTest, "CREATE TABLE test (id INT PRIMARY KEY)")

    query_result = SQL.query_get_single_result(EctoRawSQLHelpers.PostgresRepoForTest, "SELECT * FROM test")
    assert query_result == nil
  end

  test "query_get_single_return should return :error tuple when response has more than one row" do
    query_result = SQL.query_get_single_result(EctoRawSQLHelpers.PostgresRepoForTest, "SELECT * FROM generate_series(1, 2)")
  ...
  test "affecting_statement should return number of affected rows" do
    ...
    number_of_affected_rows = SQL.affecting_statement(EctoRawSQLHelpers.PostgresRepoForTest, "INSERT INTO test VALUES (1), (2)")
